...
const EMPTY_FORM = { name: '', kick_slug: '', is_active: true }
...
export default function AmigosAdminClient({ friends: initialFriends }: { friends: FriendStreamer[] }) {
  ...
  function openEdit(f: FriendStreamer) {
    setForm({ name: f.name, kick_slug: f.kick_slug, is_active: f.is_active })
    setEditId(f.id)
    setShowForm(true)
    setError(null)
  }
  ...
  function handleSubmit() {
    ...
    start(async () => {
      const result = editId
        ? await updateFriendStreamer(editId, form)
        : await createFriendStreamer(form)

      if (result?.error) { setError(result.error); return }

      if (editId) {
        setFriends(prev => prev.map(f => f.id === editId ? { ...f, ...form } : f))
      } else {
        setFriends(prev => [...prev, { id: crypto.randomUUID(), ...form }])
      }
      closeForm()
    })
  }
    ...
    <div className="space-y-4">
              ...
              <div>
                ...
                {form.kick_slug && (
                  <p className="mt-1 text-xs text-muted-foreground">
                    Canal: kick.com/<span className="text-foreground">{form.kick_slug}</span>
                  </p>
                )}
              </div>
      ...
      {friends.length === 0 ? (
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="text-xs mt-1 opacity-70">Agregá el slug de Kick y el bot los va a monitorear</p>
        </div>
      ) : (
